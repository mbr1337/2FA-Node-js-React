...
const ProfilePage = () => {
  ...
  const disableTwoFactorAuth = async (user_id: string) => {
    try {
      ...
      toast.warning("Uwierzytelnianie dwuskładnikowe wyłączone", {
        position: "top-right",
      });
    } catch (error: any) {
      store.setRequestLoading(false);
      ...
      <section className="bg-ct-blue-600  min-h-screen pt-10">
        <div className="max-w-4xl p-12 mx-auto bg-ct-dark-100 rounded-md h-[20rem] flex gap-20 justify-center items-start">
          <div className="flex-grow-2">
            <h1 className="text-2xl font-semibold">Mój Profil</h1>
            <div className="mt-8">
              <p className="mb-4">ID: {user?.id}</p>
              <p className="mb-4">Imie: {user?.name}</p>
              <p className="mb-4">Email: {user?.email}</p>
            </div>
          </div>
          <div>
            <h3 className="text-2xl font-semibold">
              Uwierzytelnianie aplikacji mobilnej (2FA)
            </h3>
            <p className="mb-4">
              Zabezpiecz swoje konto za pomocą uwierzytelniania
              dwuskładnikowego.
            </p>
